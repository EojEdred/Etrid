...
// EVM moved to ETH-PBC - Primearc Core Chain is pure coordination layer
...
// ═══════════════════════════════════════════════════════════════════════════════
// mod node_authorization_helper; // TODO: Re-enable when module exists
...
// Reserve Oracle Configuration (Primearc Core Chain - aggregates reserve data)
parameter_types! {
...
parameter_types! {
    pub const PrimearcCoreMaxPayloadSize: u32 = 1024;  // Max message size (bytes)
    pub const PrimearcCoreMessageTimeout: u32 = 1_000;  // Message expiry (blocks)
    pub const PrimearcCoreMaxPendingMessages: u32 = 1_000;  // Max queue size
    pub const PrimearcCoreIdentifier: pallet_xcm_bridge::ChainId = pallet_xcm_bridge::ChainId::PrimearcCore;
}

impl pallet_xcm_bridge::Config for Runtime {
    type RuntimeEvent = RuntimeEvent;
    type MaxPayloadSize = PrimearcCoreMaxPayloadSize;
    type MessageTimeout = PrimearcCoreMessageTimeout;
    type MaxPendingMessages = PrimearcCoreMaxPendingMessages;
    type ChainIdentifier = PrimearcCoreIdentifier;
}
...
parameter_types! {
    /// Fixed committee size of 21 validators for BFT consensus
    /// - BFT threshold: 2*21/3 + 1 = 15 signatures required
    /// - Other validators can join at epoch boundaries but committee stays at 21
    /// - This ensures optimal balance between decentralization and consensus speed
    pub const MaxCommitteeSize: u32 = 21;
    pub const MinValidatorStake: u128 = 64_000_000_000_000_000_000; // 64 ËTRID (Ivory Papers spec)
}
...
parameter_types! {
    ...
    // Fee distribution shares (in basis points, 10000 = 100%)
    pub const ConsensusParticipantFeeShareBps: u32 = 4000; // 40% of treasury fees
    pub const ConsensusFoundationShareBps: u32 = 4000; // 40% of participant pool
    pub const ConsensusDirectorsShareBps: u32 = 2000; // 20% of participant pool
    pub const ConsensusValidatorsShareBps: u32 = 2500; // 25% of participant pool
...
impl pallet_consensus_day::Config for Runtime {
    ...
    type TreasuryQuery = ConsensusDayTreasuryQuery;
    type FoundationAccount = FoundationTreasuryAccount;
    ...
    type ParticipantFeeShareBps = ConsensusParticipantFeeShareBps;
    type FoundationShareBps = ConsensusFoundationShareBps;
    type DirectorsShareBps = ConsensusDirectorsShareBps;
    type ValidatorsShareBps = ConsensusValidatorsShareBps;
    type VotersShareBps = ConsensusVotersShareBps;
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// Primearc Core Chain is the pure coordination layer with:
...
construct_runtime!(
    pub struct Runtime {
        ...
        // EVM moved to ETH-PBC - Primearc Core Chain stays pure
...
impl_runtime_apis! {
    ...
    //
    // Primearc Core Chain focuses on:
