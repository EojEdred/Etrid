...
mod asf_justification; // ASF justification types for block import
...
impl SubstrateCli for Cli {
    ...
    fn load_spec(&self, id: &str) -> Result<Box<dyn sc_service::ChainSpec>, String> {
        Ok(match id {
            ...
            "directors9" | "9directors" | "primearc-core-chain_9directors" | "primearc_9directors" => Box::new(chain_spec::directors_9_config()?),
            "session_fixed" | "session-fixed" | "primearc-core-chain_session_fixed" | "primearc_session_fixed" => Box::new(chain_spec::session_fixed_mainnet_config()?),
            "asf" | "asf_mainnet" | "primearc-core-chain_mainnet_asf" | "primearc_mainnet_asf" => Box::new(chain_spec::asf_mainnet_config()?),
            "primearc-core-chain" | "primearc" => Box::new(chain_spec::primearc_core_chain_config()?),
...
fn main() -> sc_cli::Result<()> {
    ...
    match &cli.subcommand {
        ...
        Some(Subcommand::ChainInfo(cmd)) => {
            let runner = cli.create_runner(cmd)?;
            runner.sync_run(|config| cmd.run::<primearc_core_runtime::Block>(&config))
        }
        None => {
            let runner = cli.create_runner(&cli.run)?;

            runner.run_node_until_exit(|config| async move {
                log::info!("✓ Pure ASF Consensus Mode (v119 - Full Block Sync Fix)");
