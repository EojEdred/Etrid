...
// Cumulus
// Cumulus proof size tracking disabled (removed dependency)
// Substrate
use frame_support::{
	traits::{
		...
		R: Default,
	{
		...
		let (reason, retv, used_gas, effective_gas) =
			fp_evm::handle_storage_oog::<R, _>(gas_limit, || {
				...
				let actual_proof_size = if let Some(measured_proof_size_after) = None::<u64> {
	...
	BalanceOf<T>: TryFrom<U256> + Into<U256>,
{
	...
	fn call(
		...
		let measured_proof_size_before = None::<u64>.unwrap_or_default();
	...
	fn test_evm_reentrancy() {
		TestExternalities::new_empty().execute_with(|| {
			...
			let res = Runner::<Test>::execute(
				...
				measured_proof_size_before,
				|_| {
					let measured_proof_size_before2 = None::<u64>.unwrap_or_default();
					let res = Runner::<Test>::execute(
						H160::default(),
						U256::default(),
						100_000,
