...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		/// destination_chain: 0=Primearc Core Chain, 1=PbcEdsc, 2=Other
		...
		/// source_chain: 0=Primearc Core Chain, 1=PbcEdsc, 2=Other
		/// message_type: 0=ReserveCheckpoint, 1=PriceUpdate, 2=Governance, 3=EmergencyPause, 4=Alert
		MessageReceived {
			source_chain: u8,
			message_type: u8,
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn send_checkpoint(
			origin: OriginFor<T>,
			destination_chain: u8, // 0=Primearc Core Chain, 1=PbcEdsc
			reserve_ratio: u16,
			total_reserves: u128,
			vault_value: u128,
		...
		pub fn receive_checkpoint(
			origin: OriginFor<T>,
			source_chain: u8, // 0=Primearc Core Chain, 1=PbcEdsc
			nonce: u64,
			reserve_ratio: u16,
			total_reserves: u128,
			vault_value: u128,
