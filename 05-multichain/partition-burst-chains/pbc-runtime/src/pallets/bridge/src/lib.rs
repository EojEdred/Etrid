...
//! # Bridge Pallet
//!
//! Cross-chain messaging between PBCs and Primearc Core Chain.
//!
//! ## Overview
//!
//! The Bridge pallet handles:
//! - Sending messages from PBC to Primearc Core Chain
//! - Receiving messages from Primearc Core Chain to PBC
...
pub mod pallet {
    ...
    /// Pending outgoing messages to Primearc Core Chain
    #[pallet::storage]
    ...
    /// Received incoming messages from Primearc Core Chain
    #[pallet::storage]
    ...
    /// Latest state root submitted to Primearc Core Chain
    #[pallet::storage]
    ...
    pub enum Event<T: Config> {
        /// Message sent to Primearc Core Chain
        MessageSent {
            to: u8,
            nonce: u64,
            data_hash: T::Hash,
        },
        /// Message received from Primearc Core Chain
        MessageReceived {
            from: u8,
            nonce: u64,
            data_hash: T::Hash,
        },
        /// State root aggregated to Primearc Core Chain
        StateRootAggregated {
    ...
    impl<T: Config> Pallet<T> {
        /// Send a message to Primearc Core Chain or another PBC
        #[pallet::call_index(0)]
        ...
        /// Receive a message from Primearc Core Chain (authority only)
        #[pallet::call_index(1)]
        ...
        /// Submit state root to Primearc Core Chain (authority only)
        #[pallet::call_index(2)]
