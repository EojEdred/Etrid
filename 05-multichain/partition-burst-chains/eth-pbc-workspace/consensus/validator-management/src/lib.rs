...
pub enum PeerType {
    ...
    StakingCommon,
    /// Validity Node - Minimum 64 ËTR (PBC/Primearc validator)
    ValidityNode,
...
impl PeerType {
    ...
    /// Check if this peer type can be a validator (produce blocks)
    /// DecentralizedDirectors have the HIGHEST tier rights - they can do everything
    /// ValidityNodes can do, PLUS governance powers.
    pub fn is_validator_type(&self) -> bool {
        matches!(self, PeerType::ValidityNode | PeerType::DecentralizedDirector)
    }
    ...
    pub fn can_be_in_committee(&self) -> bool {
        matches!(
            self,
            PeerType::ValidityNode | PeerType::DecentralizedDirector
        )
    }
}
...
mod tests {
    ...
    fn test_peer_type_validator_check() {
        ...
        // DecentralizedDirectors can validate too - they have ALL validator rights PLUS governance
