...
pub async fn start_collator_with_p2p(
    ...
    if p2p_config.enabled {
            log::info!("🌐 Initializing DETR P2P Network for XLM-PBC Collator...");
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    log::info!("🔧 Initializing RPC server for XLM-PBC Collator...");
...
async fn submit_state_roots(client: Arc<FullClient>) {
    log::info!("🔗 XLM-PBC: State root submitter task started");
        ...
        if best_number > last_block_number {
            ...
            match client.header(best_hash) {
                Ok(Some(header)) => {
                    ...
                    log::info!(
                        "🔗 XLM-PBC: Block #{} produced with state root: {:?}",
                        best_number,
                        state_root
                    );
                ...
                Ok(None) => {
                    log::warn!("🔗 XLM-PBC: Header not found for block #{}", best_number);
                }
                Err(e) => {
                    log::error!("🔗 XLM-PBC: Error reading header for block #{}: {:?}", best_number, e);
                }
            }
        }
    }
