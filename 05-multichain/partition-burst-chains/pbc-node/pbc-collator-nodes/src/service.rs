...
//! 2. Submits state roots to Primearc Core Chain
...
pub async fn start_collator(config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    // 2. Connect to Primearc Core Chain via RPC
...
/// Task that submits state roots to Primearc Core Chain after each block
    ...
    >>,
{
    ...
    // 3. Submit it to Primearc Core Chain's pbc_router pallet
    ...
    loop {
        ...
        if let Ok(Some(header)) = client.header(block_hash) {
            ...
            // TODO: Call Primearc Core Chain RPC to submit state root
            // primearc-core-chain_client.submit_state_root(pbc_id, block_number, state_root).await;
