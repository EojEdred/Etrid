...
fn main() -> sc_cli::Result<()> {
    ...
    match &cli.subcommand {
        ...
        None => {
            ...
            // Prepare P2P options from CLI
            let p2p_options = Some(service::CollatorP2POptions {
                enable_p2p: cli.enable_p2p,
                p2p_listen: cli.p2p_listen.clone(),
                p2p_announce: cli.p2p_announce.clone(),
                bootnodes: cli.bootnodes.clone(),
            });

            runner.run_node_until_exit(|config| async move {
                service::start_collator(config, p2p_options)
                    .await
