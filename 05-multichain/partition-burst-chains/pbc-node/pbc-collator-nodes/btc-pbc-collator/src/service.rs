...
use btc_pbc_runtime::{self, opaque::Block, RuntimeApi, AccountId};
...
pub async fn start_collator_with_p2p(
    ...
    if p2p_config.enabled {
            log::info!("🌐 Initializing DETR P2P Network for BTC-PBC Collator...");
            ...
            match p2p_cfg.initialize().await {
                Ok(p2p_network) => {
                    ...
                    if let Err(e) = p2p_service.start().await {
                        ...
                        let max_validators = btc_pbc_runtime::asf_config::AsfMaxCommitteeSize::get();
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    log::info!("🔧 Initializing RPC server for BTC-PBC Collator...");
