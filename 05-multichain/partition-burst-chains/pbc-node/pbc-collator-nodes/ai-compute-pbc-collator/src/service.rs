...
//!
//! Includes full DETR P2P integration for encrypted peer-to-peer communication.
...
use ai_compute_pbc_runtime::{self, opaque::Block, RuntimeApi, AccountId};
...
pub async fn start_collator(config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    log::info!("🌐 AI-COMPUTE-PBC: Initializing DETR P2P Network...");

    let mut p2p_config = build_p2p_config(&config, &keystore_container)?;

    // Initialize and start P2P network
    let p2p_network = if p2p_config.enabled {
        match p2p_config.initialize().await {
            Ok(network) => {
                log::info!("✅ AI-COMPUTE-PBC: DETR P2P Network initialized");
                Some(network)
            }
            Err(e) => {
                log::warn!("⚠️ AI-COMPUTE-PBC: Failed to initialize P2P network: {}", e);
                None
            }
        }
    } else {
        log::info!("⚠️ AI-COMPUTE-PBC: DETR P2P disabled via DETR_P2P_ENABLE");
        None
    };

    // Start P2P network service if initialized
    if let Some(detrp2p_network) = p2p_network {
        let p2p_service = P2PNetworkService::new(detrp2p_network.clone());
        ...
        task_manager.spawn_handle().spawn(
            ...
            async move {
                if let Err(e) = p2p_service.start().await {
                    log::error!("❌ AI-COMPUTE-PBC: P2P service error: {}", e);
                }
        ...
        let detrp2p_network = detrp2p_network.clone();
        ...
        let p2p_network_clone = detrp2p_network.clone();
        ...
        log::info!("✅ AI-COMPUTE-PBC: DETR P2P Network Service started");
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    log::info!("🔧 Initializing RPC server for AI-COMPUTE-PBC Collator...");
...
async fn submit_state_roots(client: Arc<FullClient>) {
    log::info!("🧠 AI-COMPUTE-PBC: State root submitter task started");
        ...
        if best_number > last_block_number {
            ...
            match client.header(best_hash) {
                Ok(Some(header)) => {
                    ...
                    log::info!(
                        "🧠 AI-COMPUTE-PBC: Block #{} produced with state root: {:?}",
                        best_number,
                        state_root
                    );
                ...
                Ok(None) => {
                    log::warn!("🧠 AI-COMPUTE-PBC: Header not found for block #{}", best_number);
                }
                Err(e) => {
                    log::error!("🧠 AI-COMPUTE-PBC: Error reading header for block #{}: {:?}", best_number, e);
                }
            }
        }
    }
...
async fn process_p2p_messages(
    ...
    log::info!("📨 AI-COMPUTE-PBC: P2P message processor started");
