//! Service implementation for SOL-PBC Collator
...
use sol_pbc_runtime::{self, opaque::Block, RuntimeApi, AccountId};
...
pub async fn start_collator_with_p2p(
    ...
    if let Some(p2p_config) = p2p_config_override {
        if p2p_config.enabled {
            log::info!("🌐 Initializing DETR P2P Network for SOL-PBC Collator...");
            ...
            match p2p_cfg.initialize().await {
                Ok(p2p_network) => {
                    ...
                    if let Err(e) = p2p_service.start().await {
                        ...
                        let max_validators = sol_pbc_runtime::asf_config::AsfMaxCommitteeSize::get();
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    log::info!("🔧 Initializing RPC server for SOL-PBC Collator...");
...
async fn submit_state_roots(client: Arc<FullClient>) {
    log::info!("🔗 SOL-PBC: State root submitter task started");
        ...
        if best_number > last_block_number {
            ...
            match client.header(best_hash) {
                Ok(Some(header)) => {
                    ...
                    log::info!(
                        "🔗 SOL-PBC: Block #{} produced with state root: {:?}",
                        best_number,
                        state_root
                    );
                ...
                Ok(None) => {
                    log::warn!("🔗 SOL-PBC: Header not found for block #{}", best_number);
                }
                Err(e) => {
                    log::error!("🔗 SOL-PBC: Error reading header for block #{}: {:?}", best_number, e);
                }
            }
        }
    }
