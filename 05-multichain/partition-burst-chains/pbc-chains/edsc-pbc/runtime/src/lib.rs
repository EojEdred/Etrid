...
// EDSC Checkpoint Configuration (State synchronization with Primearc Core Chain)
parameter_types! {
...
// XCM Bridge Configuration (Cross-chain messaging with Primearc Core Chain)
parameter_types! {
        ...
        fn get_preset(id: &Option<sp_genesis_builder::PresetId>) -> Option<Vec<u8>> {
            frame_support::genesis_builder_helper::get_preset::<RuntimeGenesisConfig>(id, |name| {
                match name.as_ref() {
                    ...
                    "production" | "live" => {
                        Some(include_bytes!("../presets/production.json").to_vec())
                    },
                    _ => None,
                }
            })
