...
//! between AI-Compute-PBC and Primearc Core Chain.
//!
//! ## Message Types
//! - Checkpoint submission (AI-PBC → Primearc Core Chain every 256 blocks)
...
parameter_types! {
    ...
    /// Universal location (Polkadot → Primearc Core Chain → AI-Compute-PBC)
...
pub type LocationToAccountId = (
    // Relay chain (Primearc Core Chain) account
...
pub type LocalOriginConverter = (
    // Relay chain (Primearc Core Chain) as native origin
...
parameter_types! {
    /// Relay network (Primearc Core Chain uses Polkadot's NetworkId)
...
pub type Barrier = (
    // Allow paid executions from Primearc Core Chain
...
pub type XcmRouter = (
    // Route to Primearc Core Chain (upward messages)
...
// ============================================
// CHECKPOINT SUBMISSION TO PRIMEARC_CORE_CHAIN
// ============================================

/// Helper function to submit state checkpoint to Primearc Core Chain
pub fn submit_checkpoint_to_primearc-core-chain(block_number: u32, state_root: [u8; 32]) -> DispatchResult {
    use xcm::latest::prelude::*;
    ...
    let message = Xcm(vec![
        ...
        // Step 2: Buy execution on Primearc Core Chain
        BuyExecution {
            fees: (Here, 1_000_000_000_000u128).into(),
            weight_limit: Unlimited,
        },

        // Step 3: Transact - call Primearc Core Chain's checkpoint pallet
        Transact {
    ...
    // Send to Primearc Core Chain (parent chain)
...
/// Encode checkpoint submission call for Primearc Core Chain
fn checkpoint_call_data(block_number: u32, state_root: [u8; 32]) -> Vec<u8> {
    // This would be the actual Primearc Core Chain runtime call
    // Format: pallet_index (u8) + call_index (u8) + params

    let mut call = vec![
        50, // Primearc Core Chain checkpoint pallet index (example)
...
/// Receive ËDSC from Primearc Core Chain or sibling parachain
...
pub fn offchain_worker_checkpoint_submission(block_number: u32) {
    if block_number % 256 == 0 {
        ...
        // Submit to Primearc Core Chain via XCM
        if let Err(e) = submit_checkpoint_to_primearc-core-chain(block_number, state_root) {
            log::error!("Failed to submit checkpoint at block {}: {:?}", block_number, e);
        } else {
            log::info!("✅ Checkpoint submitted to Primearc Core Chain at block {}", block_number);
        }
    }
}
