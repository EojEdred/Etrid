 ...
 *
 * Architecture:
 * - ETH PBC (EVM chain) <-> EDSC Bridge <-> Primearc Core Chain (Substrate chain)
 * - Users stake LP tokens on ETH PBC via MasterChef
 * - Rewards (ETR tokens) can be claimed on ETH PBC or bridged to Primearc Core Chain
 * - Uses EDSC burn-and-mint mechanism with M-of-N attestation (3-of-5)
 */
contract ETHPBCBridgeAdapter is ReentrancyGuard, Ownable(msg.sender) {
    ...
    /// @notice Primearc Core Chain domain identifier for EDSC
    uint32 public constant PRIMEARC_CORE_CHAIN_DOMAIN = 1;
    ...
    /**
     * @notice Bridge MasterChef rewards to Primearc Core Chain
     * @param amount Amount of ETR tokens to bridge
     * @param destinationAddress Recipient address on Primearc Core Chain (32 bytes)
     * @return transferId Unique identifier for the cross-chain transfer
     */
    function bridgeRewards(
        uint256 amount,
        bytes32 destinationAddress
    ) public nonReentrant returns (bytes32 transferId) {
        ...
        IERC20(edscToken).approve(tokenMessenger, amount);

        // Initiate cross-chain transfer via EDSC TokenMessenger
        // TokenMessenger will burn EDSC on ETH PBC and mint on Primearc Core Chain
        (bool success, ) = tokenMessenger.call(
            abi.encodeWithSignature(
                "depositForBurn(uint256,uint32,bytes32,address)",
                amount,
                PRIMEARC_CORE_CHAIN_DOMAIN,
                destinationAddress,
                edscToken
            )
        );
    ...
    /**
     * @notice Receive bridged rewards from Primearc Core Chain
     ...
     * @param bridgeToPrimearc Core Chain Whether to bridge rewards to Primearc Core Chain
     * @param destinationAddress Recipient on Primearc Core Chain (if bridging)
     */
    function harvestAndBridge(
        uint256 poolId,
        bool bridgeToPrimearc Core Chain,
        bytes32 destinationAddress
        ...
        if (bridgeToPrimearc Core Chain) {
