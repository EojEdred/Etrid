...
- ✅ Primearc Core Chain node binary (`target/release/primearc-core-chain-node`)
...
# 2. Verify binaries
ls -la bin/
ls -la target/release/primearc-core-chain-node
```
...
# Symlink for testing
cd target/release
ln -s primearc-core-chain-node eth-pbc-node
```
...
ls -la bin/zombienet bin/polkadot target/release/primearc-core-chain-node target/release/eth-pbc-node
...
pkill -f zombienet
pkill -f polkadot
pkill -f primearc-core-chain-node
