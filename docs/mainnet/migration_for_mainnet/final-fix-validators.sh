#!/bin/bash
# Final fix: Remove --rpc-external from validator configs (security issue)
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  Ëtrid FlareChain - Final Fix (Remove RPC External)        ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
fix_validator() {
    ...
    # Update systemd service WITHOUT --rpc-external (security)
...
for validator in "${VALIDATORS[@]}"; do
    ...
    echo "[VM $ip] Fixing & starting Validator $validator_num..."
...
echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  All 11 Contabo Validators Started!                         ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
echo "Next steps:"
echo "  1. Start the 2 Oracle validators (validator-1 and validator-5)"
echo "  2. Monitor sync: ssh root@<IP> journalctl -u flarechain-validator -f"
echo "  3. Network needs 15/21 validators for consensus"
echo ""
echo "Current status: 11/21 validators running (need 4 more minimum)"
