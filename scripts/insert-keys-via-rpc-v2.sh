#!/bin/bash
# Insert validator keys via RPC (v2 - with keystore cleanup)
...
echo "Inserting validator keys via RPC (with keystore cleanup)..."
echo ""
...
for i in "${!VMS[@]}"; do
    ...
    RESPONSE=$(ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no root@$VM "curl -H 'Content-Type: application/json' -d '{\\\"id\\\":1, \\\"jsonrpc\\\":\\\"2.0\\\", \\\"method\\\": \\\"author_insertKey\\\", \\\"params\\\":[\\\"asf \\\",\\\"//ValidatorNode${i}\\\",\\\"${FULL_KEY}\\\"]}' http://localhost:9944 2>/dev/null" 2>/dev/null)
...
echo ""
echo "Waiting 10 seconds for keys to be recognized..."
sleep 10
