#!/bin/bash
# Insert validator keys via RPC (v3 - with correct escaping)

SSH_KEY="$HOME/.ssh/contabo-validators"

# The 21 validator IDs
VALIDATOR_IDS=(
...
for i in "${!VMS[@]}"; do
    ...
    RESPONSE=$(ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no root@$VM 'curl -s -H "Content-Type: application/json" -d '"'"'{"id":1, "jsonrpc":"2.0", "method": "author_insertKey", "params":["asf ","//ValidatorNode'"$i"'","'"$FULL_KEY"'"]}'"'"' http://localhost:9944' 2>/dev/null)
