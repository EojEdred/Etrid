#!/bin/bash
# Update Primearc Core binary on all VMs
...
BINARY_PATH="/Users/macbook/Desktop/etrid/target/release/primearc-node"
...
for VM in $VMS; do
    ...
    cat "$BINARY_PATH" | ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "root@$VM" \
        'cat > /usr/local/bin/primearc-node && chmod +x /usr/local/bin/primearc-node'
