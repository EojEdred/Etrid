...
pallet-etwasm-vm
    ...
    └── etwasm-gas-metering (VMw system)
        ...
        │   └── Operation costs
        │
        └── re-exports from: etrid-vmw-gas (06-native-currency)
                             ↑ Canonical source for VMW economics
```

> **Note:** The `etwasm-gas-metering` module re-exports from `etrid-vmw-gas`
> (located in `06-native-currency/vmw-gas/`). VMW economics (pricing, limits,
> fee calculation) are defined in the Native Currency module as the single
> source of truth. The VM module consumes these definitions.
...
Re-exports Ëtrid's VMw (Virtual Machine Watts) gas system from the canonical source in `06-native-currency/vmw-gas/`.

**Canonical Source:** `06-native-currency/vmw-gas/src/lib.rs`
**Re-export Wrapper:** `08-etwasm-vm/gas-metering/src/lib.rs`

VMW economics (computation pricing, fee structures, block limits) are defined in the Native Currency module because they are fundamentally economic concepts. The ETWasm VM consumes these definitions for gas metering during contract execution.
