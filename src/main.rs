...
//! It can run as:
//! - Primearc Core Chain validator node (root chain with FODDoS ASF consensus)
...
//! etrid --chain primearc-core --validator
//!
...
//!
//! # Development mode
//! etrid --chain primearc-core --dev
//!
//! # Subcommands
//! etrid build-spec --chain primearc-core
//! etrid key generate
//! ```
...
// Import ASF consensus service from Primearc Core Chain
use primearc_core_node::asf_service;

/// ËTRID Chain Types
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ChainType {
    /// Primearc Core Chain - Root chain with FODDoS ASF consensus
    PrimearcCore,
...
impl ChainType {
    ...
    pub fn from_str(s: &str) -> Result<Self, String> {
        match s {
            "primearc-core" | "primearccore" => Ok(ChainType::PrimearcCore),
    ...
    pub fn runtime_name(&self) -> &'static str {
        match self {
            ChainType::PrimearcCore => "Primearc Core Chain",
    ...
    pub fn is_pbc(&self) -> bool {
        !matches!(self, ChainType::PrimearcCore)
    }
}
...
pub struct Cli {
    ...
    #[arg(
        ...
        default_value = "primearc-core",
        help = "Chain to run: primearc-core, btc-pbc, eth-pbc, sol-pbc, xlm-pbc, xrp-pbc, bnb-pbc, trx-pbc, ada-pbc, link-pbc, matic-pbc, sc-usdt-pbc, doge-pbc"
    )]
...
impl SubstrateCli for Cli {
    ...
    fn description() -> String {
        "ËTRID Multichain Protocol - Primearc Core Chain Root + PBC Collators".into()
    }
    ...
    fn load_spec(&self, id: &str) -> Result<Box<dyn sc_service::ChainSpec>, String> {
        ...
        match chain_type {
            ChainType::PrimearcCore => {
                // Load Primearc Core Chain spec
                log::info!("Loading Primearc Core Chain specification: {}", id);
                Ok(match id {
                    // Development config (single validator - Alice)
                    "dev" | "development" => Box::new(primearc_core_node::chain_spec::development_config()?),
                    // Local testnet config (Alice & Bob)
                    "local" => Box::new(primearc_core_node::chain_spec::local_testnet_config()?),
                    // Ember staging testnet (public testnet)
                    "staging" | "ember" | "testnet" => Box::new(primearc_core_node::chain_spec::staging_testnet_config()?),
                    // Primearc Core Chain mainnet (Pure ASF v108 - NO GRANDPA)
                    "" | "mainnet" | "primearccore" => Box::new(primearc_core_node::chain_spec::pure_asf_mainnet_config()?),
                    // Path to custom chainspec JSON
                    path => Box::new(primearc_core_node::chain_spec::ChainSpec::from_json_file(
...
fn main() -> sc_cli::Result<()> {
    ...
    if cli.subcommand.is_none() {
        ...
        if chain_type.is_pbc() {
            ...
            // Primearc Core Chain must use --validator
            if !cli.run.validator {
                return Err(sc_cli::Error::Input(
                    "Primearc Core Chain requires --validator flag".into()
                ));
            }
            if cli.collator {
                return Err(sc_cli::Error::Input(
                    "Cannot use --collator with Primearc Core Chain. Use --validator instead.".into()
                ));
            }
        }
    }
    ...
    match &cli.subcommand {
        ...
        None => {
            ...
            match chain_type {
                ChainType::PrimearcCore => {
                    log::info!("🔥 Starting Primearc Core Chain validator node...");
                    log::info!("⚙️  Consensus: FODDoS ASF Algorithm");
                    log::info!("🌐 Network: Custom DETR P2P + Substrate Networking");

                    // Start Primearc Core Chain with ASF consensus service
                    runner.run_node_until_exit(|config| async move {
                        log::info!("🔥 Starting Primearc Core Chain with ASF consensus...");
                ...
                _ => {
                    ...
                    log::info!("🔗 Relay: Connected to Primearc Core Chain");
                    log::info!("🌐 Network: Cumulus + Custom DETR P2P");

                    // TODO: Create PBC ASF services similar to Primearc Core Chain
                    ...
                    runner.run_node_until_exit(|config| async move {
                        ...
                        log::error!("TODO: Create PBC ASF services similar to Primearc Core Chain");
...
// 1. Import primearc-core service module
...
// 1. Wire up FODDoS ASF consensus for Primearc Core Chain
...
// 2. Set up relay between Primearc Core Chain and PBCs
