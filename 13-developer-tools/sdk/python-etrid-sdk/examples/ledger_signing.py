...
def main():
    ...
    try:
        ...
        print("    ✓ Connected to Primearc Core Chain")
    except Exception as e:
        print(f"    ✗ Failed to connect to Primearc Core Chain: {e}")
        disconnect_ledger(device)
        return 1
    ...
    # Step 6: Submit transaction to Primearc Core Chain
    print("[6] Submitting transaction to Primearc Core Chain...")
    ...
    try:
        ...
        print(f"    ✓ View on explorer: https://explorer.etrid.org/tx/{tx_hash}")
        print()
    except Exception as e:
...
def submit_signed_transaction(client, tx_data, signature):
    """
    Submit a signed transaction to Primearc Core Chain.
