...
async function stakingExample() {
  const api = await ApiPromise.create({ provider: new WsProvider('wss://ws.etrid.org/primearc') });
  const keyring = new Keyring({ type: 'sr25519' });
...
For more information, visit the [Etrid Documentation](https://docs.etrid.org) or join our [Discord Community](https://discord.gg/etrid).
