# EVM Web Integration (staking + wallet) - Implementation Notes

## Overview

This document covers the EVM wallet connection (RainbowKit/wagmi) and MasterChef LP staking UI used by `/evm` and `/staking/eth-pbc`.

Note: **Primearc Core (Substrate)** is the main chain for the console; the EVM integration is an optional module and should be configured to target the correct EVM network via environment variables.
...
- **Status:** ✅ Created
- **Contents:** EVM chain definition (`primearcCoreEvm`) driven by `NEXT_PUBLIC_EVM_*` env vars (with fallbacks for legacy `NEXT_PUBLIC_ETH_PBC_*` vars)

#### Wagmi Configuration
- **File:** `src/config/wagmi.ts`
- **Status:** ✅ Created
- **Contents:** RainbowKit wagmi config pointing at `primearcCoreEvm`
...
- **Source:** `/Users/macbook/Desktop/etrid/contracts/primeswap/artifacts/src/farming/MasterChef.sol/MasterChef.json`
...
#### EVM Staking Page (`/staking/eth-pbc`)
...
### New Files (11)
1. `src/config/chains.ts` - EVM chain definition (env-driven)
...
Web3 is provided client-side via `src/components/providers/Web3Provider.tsx` and is mounted from EVM pages using `next/dynamic({ ssr: false })`.
...
2. **Wait for Contract Deployment**
   - Backend team must deploy the MasterChef contract to the target EVM chain
   - Update `src/config/contracts.ts` with deployed address
   - Update LP token address in `app/staking/eth-pbc/page.tsx`

3. **Ensure an EVM JSON-RPC endpoint is available**
   - Set `NEXT_PUBLIC_EVM_RPC_HTTP_URL` in `.env.local` (default example: `http://127.0.0.1:8545`)
   - Verify chain ID matches `NEXT_PUBLIC_EVM_CHAIN_ID`

4. **Update Chain ID (if needed)**
   - Check actual EVM chain ID from your JSON-RPC:
     ```bash
   ...
   - Update `.env.local` if different:
     ```
     NEXT_PUBLIC_EVM_CHAIN_ID=YOUR_CHAIN_ID
     ```
