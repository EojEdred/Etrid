...
import { getPrimearcWsEndpoints } from '@/lib/runtime-config';
...
export function ChainSelectorProvider({
    ...
    try {
      ...
      const endpoint =
        chain.id === 'primearc-core'
          ? (getPrimearcWsEndpoints()[0] ?? chain.wsEndpoint)
          : chain.wsEndpoint;

      console.log(`[ChainSelector] Connecting to ${chain.name} (${endpoint})...`);
      ...
      const tokenDecimalsValue = properties.tokenDecimals.unwrapOr([chain.decimals])[0];
      const tokenDecimals = typeof tokenDecimalsValue === 'number'
        ? tokenDecimalsValue
        : tokenDecimalsValue.toNumber ? tokenDecimalsValue.toNumber() : Number(tokenDecimalsValue.toString());
  ...
  const getBalance = useCallback(async (address: string): Promise<BalanceData | null> => {
    ...
    try {
      ...
      // Extract balance data properly for Polkadot.js API
      const accountData = accountInfo.toHuman();
      const data = accountInfo.toJSON() as any;
      const balanceData = data.data;

      const free = balanceData.free || '0';
      const reserved = balanceData.reserved || '0';
      const frozen = balanceData.frozen || '0';
      const total = (BigInt(free.toString()) + BigInt(reserved.toString())).toString();
