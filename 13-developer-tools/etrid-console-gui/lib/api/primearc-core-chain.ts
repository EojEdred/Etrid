...
export class PrimearcCoreChainAPI {
  ...
  async getBalance(chainId: string, address: string): Promise<string> {
    ...
    const balanceNum = parseInt(free.toString()) / Math.pow(10, decimals);
  ...
  async getStakingInfo(address: string): Promise<any> {
    ...
    const validatorsJson = validators.toJSON() as any[];
    const isValidator = Array.isArray(validatorsJson) && validatorsJson.some((v) => v.toString() === address);
