...
└── vmw-gas/               (CANONICAL SOURCE - gas metering + fees)
    ├── Cargo.toml
    └── src/lib.rs        (~600 lines)
```

## Architecture: Canonical Source Pattern

VMW (Virtual Machine Watts) is an **economic concept** - it represents computation pricing, fee calculations, and conversion to ÉTR. As such, the canonical source lives in the Native Currency module (06), not in the VM (08).

```
06-native-currency/vmw-gas/   ← Canonical source (VMW economics)
         ↑
08-etwasm-vm/gas-metering/    ← Re-exports for VM consumers
         ↑
...
- VMW pricing is economics, not VM implementation
- Single source of truth prevents drift between modules
- VM consumers get consistent economics via re-export
- Changes to VMW economics propagate automatically
...
**For VM-related pallets (08-etwasm-vm consumers):**

```toml
[dependencies]
etrid-economics = { path = "../../06-native-currency/economics", default-features = false }
etwasm-gas-metering = { path = "../gas-metering", default-features = false }
```

```rust
use etrid_economics::{ Balance, CurrencyType, ONE_ETRID };
use etwasm_gas_metering::{ VMw, GasMeter, VMW_BLOCK_LIMIT };
```

**For non-VM pallets (direct economics access):**
...
> **Note:** `etwasm-gas-metering` re-exports everything from `etrid-vmw-gas`, so the APIs are identical.
...
**Module Hierarchy:**
```
etrid-economics (06)     ← Currency denominations, supply management
etrid-vmw-gas (06)       ← CANONICAL: VMW economics, gas costs, fee calculation
    └─► etwasm-gas-metering (08)  ← Re-export for VM consumers
            └─► etwasm-opcodes (08)
            └─► etwasm-runtime (08)
            └─► etwasm-pallet (08)
```
...
**For VM consumers:** Import `etwasm-gas-metering` which re-exports from the canonical source.
**For direct economics access:** Import `etrid-vmw-gas` directly from 06-native-currency.
