...
2. **Runtime**: Primearc Core Chain runtime at `runtime/flare-chain/`
...
Edit `/Users/macbook/Desktop/etrid/runtime/flare-chain/Cargo.toml`:
...
Edit `/Users/macbook/Desktop/etrid/runtime/flare-chain/src/lib.rs`:
...
Edit `/Users/macbook/Desktop/etrid/runtime/flare-chain/src/genesis.rs` (or where genesis is configured):

```rust
use pallet_director_election::GenesisConfig as DirectorElectionConfig;

pub fn flarechain_genesis(
    // ... existing parameters
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: create_runtime_str!("flarechain"),
    impl_name: create_runtime_str!("flarechain"),
    authoring_version: 1,
...
2. **Generate Wasm**:
   ```bash
   # Wasm blob will be in target/release/wbuild/flarechain-runtime/
   ```

3. **Runtime Upgrade**:
   ```javascript
   // Via governance
   const wasmBlob = fs.readFileSync('flarechain_runtime.compact.compressed.wasm');
