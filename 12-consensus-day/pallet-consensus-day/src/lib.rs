...
pub mod pallet {
    ...
    /// Treasury query interface for getting accumulated fee balances
    pub trait TreasuryQueryInterface<Balance> {
        /// Get total accumulated transaction fees in treasury
        fn get_transaction_fee_balance() -> Balance;
        /// Withdraw funds from treasury for participant distribution
        fn withdraw_for_distribution(amount: Balance) -> DispatchResult;
    }

    /// No-op implementation for testing
    impl<Balance: Default> TreasuryQueryInterface<Balance> for () {
        fn get_transaction_fee_balance() -> Balance { Balance::default() }
        fn withdraw_for_distribution(_amount: Balance) -> DispatchResult { Ok(()) }
    }
    ...
    pub trait Config: frame_system::Config {
        ...
        /// Treasury query interface for fee balance queries
        type TreasuryQuery: TreasuryQueryInterface<BalanceOf<Self>>;

        /// Foundation account for fee distribution
        ...
        /// Percentage of treasury fees for participant distribution (default 40% = 4000 bps)
        #[pallet::constant]
        type ParticipantFeeShareBps: Get<u32>;

        /// Foundation share of participant pool (default 40% = 4000 bps)
        #[pallet::constant]
        type FoundationShareBps: Get<u32>;

        /// Directors share of participant pool (default 20% = 2000 bps)
        #[pallet::constant]
        type DirectorsShareBps: Get<u32>;

        /// Validators share of participant pool (default 25% = 2500 bps)
        #[pallet::constant]
        type ValidatorsShareBps: Get<u32>;

        /// Voters share of participant pool (default 15% = 1500 bps)
        #[pallet::constant]
        type VotersShareBps: Get<u32>;
    }
    ...
    /// Total accumulated transaction fees distributed during current Consensus Day
    #[pallet::storage]
    #[pallet::getter(fn fees_distributed)]
    pub type FeesDistributed<T: Config> = StorageValue<_, BalanceOf<T>, ValueQuery>;

    /// Fee distribution breakdown for current Consensus Day
    #[pallet::storage]
    #[pallet::getter(fn fee_distribution_breakdown)]
    pub type FeeDistributionBreakdown<T: Config> = StorageValue<
        _,
        (BalanceOf<T>, BalanceOf<T>, BalanceOf<T>, BalanceOf<T>), // (foundation, directors, validators, voters)
        ValueQuery,
    >;
    ...
    pub enum Event<T: Config> {
        ...
        /// Fee-based distribution executed (hybrid model)
        /// [total_fees_distributed, foundation_share, directors_share, validators_share, voters_share]
        FeeDistributionExecuted {
            total_fees: BalanceOf<T>,
            foundation_share: BalanceOf<T>,
            directors_share: BalanceOf<T>,
            validators_share: BalanceOf<T>,
            voters_share: BalanceOf<T>,
        },
        /// Foundation received fee distribution [amount]
        FoundationFeeDistributed(BalanceOf<T>),
        /// Director received fee distribution [account, amount]
        DirectorFeeDistributed(T::AccountId, BalanceOf<T>),
        /// Validator received fee distribution [account, amount]
        ValidatorFeeDistributed(T::AccountId, BalanceOf<T>),
        /// Voter received fee distribution (added to participation rewards) [account, amount]
        VoterFeeDistributed(T::AccountId, BalanceOf<T>),
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// Execute distribution of participation rewards + hybrid fee distribution
        ///
        /// This implements the FODDOS hybrid model:
        /// 1. Minting-based participation rewards (1% of newly minted tokens)
        /// 2. Fee-based distribution (40% of accumulated treasury fees):
        ///    - Foundation: 40% of participant pool
        ///    - Directors: 20% of participant pool
        ///    - Validators: 25% of participant pool
        ///    - Voters: 15% of participant pool (added to participation rewards)
        fn execute_distribution() -> DispatchResult {
            let total_minted = TotalMinted::<T>::get();
            let total_proposals = NextProposalId::<T>::get();

            // ════════════════════════════════════════════════════════════════
            // PART 1: Minting-based participation rewards (1% of total minted)
            // ════════════════════════════════════════════════════════════════
            let minting_participation_pool = total_minted / 100u32.into();

            // Calculate total voting power used
            let mut total_voting_power = 0u128;
            let mut voters: Vec<(T::AccountId, u128, u32)> = Vec::new(); // (account, voting_power, proposals_voted)
            ...
            for (voter, _) in VotingPowerMap::<T>::iter() {
                ...
                if proposals_voted > 0 {
                    ...
                    voters.push((voter, vp_info.voting_power, proposals_voted));
                }
            }

            // ════════════════════════════════════════════════════════════════
            // PART 2: Fee-based distribution (40% of accumulated fees)
            // ════════════════════════════════════════════════════════════════
            let accumulated_fees = T::TreasuryQuery::get_transaction_fee_balance();
            let participant_fee_share_bps = T::ParticipantFeeShareBps::get(); // Default 4000 = 40%

            // Calculate 40% of accumulated fees for participant distribution
            let fee_participant_pool = Self::calculate_bps(accumulated_fees, participant_fee_share_bps);

            // Calculate individual shares from fee pool
            let foundation_share_bps = T::FoundationShareBps::get();   // 4000 = 40%
            let directors_share_bps = T::DirectorsShareBps::get();     // 2000 = 20%
            let validators_share_bps = T::ValidatorsShareBps::get();   // 2500 = 25%
            let voters_share_bps = T::VotersShareBps::get();           // 1500 = 15%

            let foundation_fee_amount = Self::calculate_bps(fee_participant_pool, foundation_share_bps);
            let directors_fee_amount = Self::calculate_bps(fee_participant_pool, directors_share_bps);
            let validators_fee_amount = Self::calculate_bps(fee_participant_pool, validators_share_bps);
            let voters_fee_amount = Self::calculate_bps(fee_participant_pool, voters_share_bps);

            // Withdraw funds from treasury for distribution (if any)
            if !fee_participant_pool.is_zero() {
                let _ = T::TreasuryQuery::withdraw_for_distribution(fee_participant_pool);
            }

            // ════════════════════════════════════════════════════════════════
            // PART 3: Distribute to Foundation
            // ════════════════════════════════════════════════════════════════
            if !foundation_fee_amount.is_zero() {
                let foundation_account = T::FoundationAccount::get();
                T::Currency::deposit_creating(&foundation_account, foundation_fee_amount);
                Self::deposit_event(Event::FoundationFeeDistributed(foundation_fee_amount));
            }

            // ════════════════════════════════════════════════════════════════
            // PART 4: Distribute to Directors (split among 9)
            // ════════════════════════════════════════════════════════════════
            let elected_directors = ElectedDirectors::<T>::get();
            let director_count = elected_directors.len() as u128;
            if director_count > 0 && !directors_fee_amount.is_zero() {
                let per_director = Self::balance_to_u128(directors_fee_amount) / director_count;
                for director in elected_directors.iter() {
                    let amount = Self::u128_to_balance(per_director);
                    T::Currency::deposit_creating(director, amount);
                    Self::deposit_event(Event::DirectorFeeDistributed(director.clone(), amount));
                }
            }

            // ════════════════════════════════════════════════════════════════
            // PART 5: Distribute to Validators (split among active validators)
            // ════════════════════════════════════════════════════════════════
            let validator_count = ActiveValidatorCount::<T>::get() as u128;
            if validator_count > 0 && !validators_fee_amount.is_zero() {
                let per_validator = Self::balance_to_u128(validators_fee_amount) / validator_count;
                for (validator, is_active) in Validators::<T>::iter() {
                    if is_active {
                        let amount = Self::u128_to_balance(per_validator);
                        T::Currency::deposit_creating(&validator, amount);
                        Self::deposit_event(Event::ValidatorFeeDistributed(validator, amount));
                    }
                }
            }

            // ════════════════════════════════════════════════════════════════
            // PART 6: Distribute participation rewards (minting + voter fees)
            // ════════════════════════════════════════════════════════════════
            if total_voting_power > 0 {
                // Combine minting pool + voter fee share
                let total_voter_pool = Self::balance_to_u128(minting_participation_pool)
                    .saturating_add(Self::balance_to_u128(voters_fee_amount));

                for (voter, voting_power, proposals_voted) in voters.iter() {
                    let voter_share = (*voting_power as u128) * total_voter_pool / total_voting_power;
                    ...
                    // Also emit voter fee distribution event for the fee portion
                    if !voters_fee_amount.is_zero() {
                        let fee_portion = (*voting_power as u128) * Self::balance_to_u128(voters_fee_amount) / total_voting_power;
                        Self::deposit_event(Event::VoterFeeDistributed(
                            voter.clone(),
                            Self::u128_to_balance(fee_portion),
                        ));
                    }
            ...
            // ════════════════════════════════════════════════════════════════
            // PART 7: Record fee distribution and emit summary event
            // ════════════════════════════════════════════════════════════════
            FeesDistributed::<T>::put(fee_participant_pool);
            FeeDistributionBreakdown::<T>::put((
                foundation_fee_amount,
                directors_fee_amount,
                validators_fee_amount,
                voters_fee_amount,
            ));

            if !fee_participant_pool.is_zero() {
                Self::deposit_event(Event::FeeDistributionExecuted {
                    total_fees: fee_participant_pool,
                    foundation_share: foundation_fee_amount,
                    directors_share: directors_fee_amount,
                    validators_share: validators_fee_amount,
                    voters_share: voters_fee_amount,
                });
            }

            // ════════════════════════════════════════════════════════════════
            // PART 8: Elect top 9 directors for next term
            // ════════════════════════════════════════════════════════════════
        ...
        /// Calculate basis points (bps) of a balance
        /// 10000 bps = 100%
        fn calculate_bps(amount: BalanceOf<T>, bps: u32) -> BalanceOf<T> {
            let amount_u128 = Self::balance_to_u128(amount);
            let result = amount_u128.saturating_mul(bps as u128) / 10000u128;
            Self::u128_to_balance(result)
        }
