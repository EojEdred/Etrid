...
/**
 * @title EDSC - Ëtrid Dollar Stablecoin on Polygon
 ...
 */
contract EdscPolygon is ERC20, ERC20Burnable, Ownable {
        ...
        Ownable(initialOwner)
    {
        ...
        // 100,000 EDSC for initial EDSC/USDC pool on Polygon
     ...
     * @param _polygonBridge Address of Base bridge contract
     */
    function setPolygonBridge(address _polygonBridge) external onlyOwner {
        if (_polygonBridge == address(0)) revert ZeroAddress();
    ...
    function bridgeMint(address to, uint256 amount, bytes32 txHash)
        ...
        whenNotPaused
    {
        if (msg.sender != polygonBridge && msg.sender != flarechainBridge) {
            revert OnlyBridge();
        }
     ...
     * @return Current total supply of EDSC on Polygon
     */
