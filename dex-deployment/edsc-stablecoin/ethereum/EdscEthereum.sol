...
/**
 * @title EDSC - Ëtrid Dollar Stablecoin on Ethereum
 ...
 */
contract EdscEthereum is ERC20, ERC20Burnable, Ownable {
    ...
    event EthereumBridgeUpdated(address indexed oldBridge, address indexed newBridge);
        ...
        Ownable(initialOwner)
    {
        ...
        // 100,000 EDSC for initial EDSC/USDC pool on Ethereum
     ...
     * @param _ethereumBridge Address of Base bridge contract
     */
    function setEthereumBridge(address _ethereumBridge) external onlyOwner {
        if (_ethereumBridge == address(0)) revert ZeroAddress();

        address oldBridge = ethereumBridge;
        ethereumBridge = _ethereumBridge;

        emit EthereumBridgeUpdated(oldBridge, _ethereumBridge);
    }
    ...
    function bridgeMint(address to, uint256 amount, bytes32 txHash)
        ...
        whenNotPaused
    {
        if (msg.sender != ethereumBridge && msg.sender != flarechainBridge) {
            revert OnlyBridge();
        }
     ...
     * @return Current total supply of EDSC on Ethereum
     */
