...
async function main() {
  console.log("\n═══════════════════════════════════════════════════════════");
  console.log("  EDSC (Ëtrid Dollar Stablecoin) Deployment - BSC");
  console.log("═══════════════════════════════════════════════════════════\n");
  ...
  console.log("🚀 Deploying uEdscBSC contract...");
  const uEdscBSC = await hre.ethers.getContractFactory("uEdscBSC");
  const edsc = await uEdscBSC.deploy(foundationMultisig);
  await edsc.waitForDeployment();

  const edscAddress = await edsc.getAddress();
  console.log("✅ uEdscBSC deployed to:", edscAddress);
