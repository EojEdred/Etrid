 ...
 */
contract EdscBSC is ERC20, ERC20Burnable, Ownable {
    ...
    event BSCBridgeUpdated(address indexed oldBridge, address indexed newBridge);
     ...
     * @param _bscBridge Address of Base bridge contract
     */
    function setBSCBridge(address _bscBridge) external onlyOwner {
        if (_bscBridge == address(0)) revert ZeroAddress();

        address oldBridge = bscBridge;
        bscBridge = _bscBridge;

        emit BSCBridgeUpdated(oldBridge, _bscBridge);
    }
    ...
    function bridgeMint(address to, uint256 amount, bytes32 txHash)
        ...
        whenNotPaused
    {
        if (msg.sender != bscBridge && msg.sender != flarechainBridge) {
            revert OnlyBridge();
        }
     ...
     * @return Current total supply of EDSC on BSC
     */
