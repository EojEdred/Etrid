...
async function main() {
  console.log("\n═══════════════════════════════════════════════════════════");
  console.log("  EDSC (Ëtrid Dollar Stablecoin) Deployment - Arbitrum");
  console.log("═══════════════════════════════════════════════════════════\n");
  ...
  console.log("🚀 Deploying uEdscArbitrum contract...");
  const uEdscArbitrum = await hre.ethers.getContractFactory("uEdscArbitrum");
  const edsc = await uEdscArbitrum.deploy(foundationMultisig);
  await edsc.waitForDeployment();

  const edscAddress = await edsc.getAddress();
  console.log("✅ uEdscArbitrum deployed to:", edscAddress);
