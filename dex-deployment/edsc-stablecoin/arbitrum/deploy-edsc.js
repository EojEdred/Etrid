...
async function main() {
  ...
  console.log("🚀 Deploying EdscuArbitrum contract...");
  const EdscuArbitrum = await hre.ethers.getContractFactory("EdscuArbitrum");
  const edsc = await EdscuArbitrum.deploy(foundationMultisig, reserveVault);
  await edsc.waitForDeployment();

  const edscAddress = await edsc.getAddress();
  console.log("✅ EdscuArbitrum deployed to:", edscAddress);
