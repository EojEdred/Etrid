...
/**
 * @title EtridPoly
 * @dev ÉTR Token on Polygon (ERC-20)
 *
 * Official ÉTR token deployment for Polygon PoS chain.
 * Designed for QuickSwap V3 integration and low-cost DeFi operations.
 *
 * Token Specifications:
 * - Name: Ëtrid Coin
 * - Symbol: ÉTR
 * - Decimals: 18 (Polygon/EVM standard)
 * - Initial Supply: 15M ÉTR (for QuickSwap V3 liquidity)
 ...
 */
contract EtridPoly is ERC20, ERC20Burnable, Ownable {

    /// @dev Maximum supply cap (1 billion ÉTR)
    uint256 public constant MAX_SUPPLY = 1_000_000_000 * 10**18;

    /// @dev Polygon PoS Bridge contract address (for Ethereum ↔ Polygon bridging)
    address public polygonBridge;

    /// @dev Cross-chain bridge contract address (for other chains)
    address public crossChainBridge;

    /// @notice Emitted when Polygon bridge is updated
    event PolygonBridgeUpdated(address indexed oldBridge, address indexed newBridge);
    ...
    /**
     * @dev Constructor mints initial supply for QuickSwap V3 liquidity
     * @param initialOwner Foundation multisig address
     */
    constructor(address initialOwner) ERC20(unicode"Ëtrid", unicode"ËTR") Ownable(initialOwner) {
        // Mint 100K ÉTR for bootstrap liquidity (expand later)
        _mint(initialOwner, 100_000 * 10**18);
    }

    /**
     * @dev Set the Polygon PoS bridge contract address
     * @param _polygonBridge Address of the official Polygon bridge
     *
     * Requirements:
     * - Caller must be owner (Foundation multisig)
     * - Bridge address cannot be zero
     */
    function setPolygonBridge(address _polygonBridge) external onlyOwner {
        require(_polygonBridge != address(0), "Bridge cannot be zero address");
        address oldBridge = polygonBridge;
        polygonBridge = _polygonBridge;
        emit PolygonBridgeUpdated(oldBridge, _polygonBridge);
    }
    ...
    /**
     * @dev Mint tokens when bridging TO Polygon
     * @param to Recipient address on Polygon
     ...
     *
     * @notice This is called when users lock ÉTR on another chain and mint on Polygon
     */
    function bridgeMint(address to, uint256 amount, string memory sourceChain) external {
        require(
            msg.sender == polygonBridge || msg.sender == crossChainBridge,
    ...
    /**
     * @dev Burn tokens when bridging FROM Polygon
     ...
     *
     * @notice This is called when users burn ÉTR on Polygon to unlock on another chain
     */
