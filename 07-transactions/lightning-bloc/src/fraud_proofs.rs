...
//!
//! This module uses Blake2b-256 for state hashing to ensure collision resistance and
//! Ed25519 signatures for authentication of fraud proofs and challenge responses.
...
// Cryptographic hashing for state verification
...
impl StateTransition {
    ...
    /// Calculate state hash for verification using Blake2b-256
    ///
    /// This produces a cryptographically secure 32-byte hash of the state transition,
    /// suitable for on-chain verification and fraud proof validation.
    pub fn state_hash(&self) -> Vec<u8> {
        // Use Blake2b-256 for cryptographic state hashing
        ...
        // Hash nonces (little-endian encoding)
        hasher.update(&self.from_nonce.to_le_bytes());
        hasher.update(&self.to_nonce.to_le_bytes());

        // Hash balances (little-endian encoding for u128)
        hasher.update(&self.from_balance_a.to_le_bytes());
        hasher.update(&self.from_balance_b.to_le_bytes());
        hasher.update(&self.to_balance_a.to_le_bytes());
        hasher.update(&self.to_balance_b.to_le_bytes());
    ...
    /// Calculate commitment hash for channel state (used in HTLC)
...
pub struct FraudProof {
    ...
    /// Ed25519 signature over the state transition commitment
    pub signature: Vec<u8>,
    /// Ed25519 public key of the challenger (32 bytes)
...
impl FraudProof {
    ...
    ///
    /// # Arguments
    /// * `channel_id` - The channel being disputed
    /// * `challenger` - Address/ID of the party submitting the fraud proof
    /// * `invalid_transition` - The state transition claimed to be invalid
    /// * `proof_data` - Additional evidence (e.g., merkle proofs, witness data)
    /// * `signature` - Ed25519 signature over the state commitment (64 bytes)
    /// * `public_key` - Ed25519 public key of the challenger (32 bytes)
    ...
    pub fn new(
        ...
        if signature.len() != 64 {
            return Err(FraudProofError::InvalidSignatureLength {
                expected: 64,
                got: signature.len(),
            });
        }
        ...
        if public_key.len() != 32 {
            return Err(FraudProofError::InvalidPublicKeyLength {
                expected: 32,
                got: public_key.len(),
            });
        }
    ...
    /// Verify fraud proof validity including Ed25519 signature verification
    ///
    /// This performs:
    /// 1. State transition validation (nonce increment, balance conservation)
    /// 2. Proof data integrity check
    ...
    pub fn verify(&self) -> Result<(), FraudProofError> {
        ...
        let public_key_bytes: [u8; 32] = self.public_key.as_slice()
            .try_into()
            .map_err(|_| FraudProofError::InvalidPublicKey)?;
        ...
        let signature_bytes: [u8; 64] = self.signature.as_slice()
            .try_into()
            .map_err(|_| FraudProofError::InvalidSignature)?;

        let signature = Signature::from_bytes(&signature_bytes);

        // 3. Get the message (state commitment hash)
        let message = self.invalid_transition.commitment_hash();

        // 4. Verify the signature
        verifying_key
            .verify(&message, &signature)
            .map_err(|_| FraudProofError::SignatureVerificationFailed)?;
...
pub enum FraudProofError {
    ...
    EmptyProof,
    InvalidSignature,
    /// Signature length invalid (expected 64 bytes for Ed25519)
    InvalidSignatureLength { expected: usize, got: usize },
    /// Public key length invalid (expected 32 bytes for Ed25519)
...
impl fmt::Display for FraudProofError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            FraudProofError::InvalidSignature => write!(f, "Invalid signature format"),
            FraudProofError::InvalidSignatureLength { expected, got } => {
                write!(f, "Invalid signature length: expected {} bytes, got {}", expected, got)
            }
            FraudProofError::InvalidPublicKeyLength { expected, got } => {
                write!(f, "Invalid public key length: expected {} bytes, got {}", expected, got)
            }
            FraudProofError::InvalidPublicKey => write!(f, "Invalid Ed25519 public key"),
            FraudProofError::SignatureVerificationFailed => {
