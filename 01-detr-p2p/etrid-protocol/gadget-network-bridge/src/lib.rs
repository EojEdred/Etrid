...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct VoteData {
    pub validator_id: [u8; 32],  // V9: Full 32-byte AccountId32
    pub view: u64,
...
pub struct ViewChangeData {
    pub new_view: u64,
    pub sender: [u8; 32],  // V9: Full 32-byte AccountId32
}
                    ...
                    while let Some(msg) = self.bridge.get_inbound_message().await {
                        match msg {
                            ConsensusBridgeMessage::Vote(vote) => {
                                ...
                                println!(
                                    "📥 Forwarding vote from validator {:02x}{:02x}..{:02x}{:02x} (view {}) to finality-gadget",
                                    vote.validator_id[0], vote.validator_id[1],
                                    vote.validator_id[30], vote.validator_id[31],
                                    vote.view
                                );
                            ...
                            ConsensusBridgeMessage::ViewChange(view_change) => {
                                ...
                                println!(
                                    "📥 Forwarding view change to {} from validator {:02x}{:02x}..{:02x}{:02x}",
                                    view_change.new_view,
                                    view_change.sender[0], view_change.sender[1],
                                    view_change.sender[30], view_change.sender[31]
                                );
...
mod tests {
    ...
    async fn test_inbound_vote_routing() {
        ...
        let vote = VoteData {
            validator_id: [1u8; 32],  // V9: Full 32-byte validator ID
            view: 0,
            block_hash: [0u8; 32],
            signature: vec![],
        };
        ...
        match msg.unwrap() {
            ConsensusBridgeMessage::Vote(v) => {
                assert_eq!(v.validator_id, [1u8; 32]);
            }
