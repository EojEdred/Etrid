...
This version supports BOTH Claude API and local LLMs via Ollama.
...
To use Claude API ($100-150/month):
- Set in .env: ANTHROPIC_API_KEY=your_key
- Leave USE_LOCAL_LLM=false or unset
"""
...
class ContentGenerator:
    def __init__(self, api_key: Optional[str] = None):
        ...
        if self.use_local_llm:
            ...
            # Test connection
            try:
                response = requests.get(f"{self.local_llm_url}/api/tags", timeout=2)
                if response.status_code == 200:
                    print(f"✅ Using local LLM: {self.local_llm_model} at {self.local_llm_url}")
                else:
                    print(f"⚠️  Local LLM server not responding, will use mock content")
            except Exception as e:
                print(f"⚠️  Cannot connect to Ollama at {self.local_llm_url}")
                print(f"   Make sure Ollama is running: ollama serve")
                print(f"   Will use mock content generation")
        else:
            # Use Claude API
    ...
    async def _generate_with_ollama(
        ...
        """Generate content using local Ollama LLM (FREE)"""
        try:
            ...
            response = requests.post(
                ...
                timeout=60  # Longer timeout for slower hardware
            )
            ...
            print(f"⚠️  Ollama request timed out (model may be slow on this hardware)")
            return self._generate_mock_content(prompt, "oracle-dev")
        except requests.exceptions.ConnectionError:
            print(f"❌ Cannot connect to Ollama - is it running? (ollama serve)")
            return self._generate_mock_content(prompt, "oracle-dev")
        except Exception as e:
...
Generate content that {persona['name']} would write."""
        ...
        if self.use_local_llm:
            return await self._generate_with_ollama(prompt, system_prompt, max_tokens, temperature)
    ...
    def _generate_mock_content(self, prompt: str, dev: str) -> str:
        """Generate mock content when API/LLM is unavailable"""
        persona = self.personas.get(dev, self.personas["oracle-dev"])
...
Return ONLY the tweets, one per line, numbered 1-{num_tweets}."""
        ...
        if not self.client and not self.use_local_llm:
            # Mock thread
            return [
                f"🧵 Thread: {topic}",
...
- Include relevant links if helpful (etrid.org/docs, etc.)
"""
