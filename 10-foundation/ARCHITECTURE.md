...
### 3. Treasury (`pallets/pallet-treasury/`)

The Treasury is the central fund management system for Ëtrid, receiving fees from multiple sources and distributing funds during Consensus Day.

#### Fee Collection Architecture

```text
╔═══════════════════════════════════════════════════════════════════════════════╗
║                        TREASURY FEE COLLECTION PIPELINE                        ║
╚═══════════════════════════════════════════════════════════════════════════════╝

YEAR-ROUND FEE SOURCES:
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│  ┌───────────────┐    ┌───────────────┐    ┌───────────────┐              │
│  │  VMw Gas Fees │    │ Bridge Fees   │    │ EDSC Stability│              │
│  │ (50% to Treas)│    │ (10% to Treas)│    │ (100% to Treas)             │
│  └───────┬───────┘    └───────┬───────┘    └───────┬───────┘              │
│          │                    │                    │                       │
│          │       ┌───────────┐│┌───────────┐      │                       │
│          │       │ Slashing  ││ │ Foundation│      │                       │
...
#### Treasury Interface (Fee Reception)

```rust
/// Interface for receiving fees from pallets
pub trait TreasuryInterface<Balance> {
    /// Receive transaction fees from ETWasm or token pallet
    fn receive_transaction_fees(amount: Balance) -> DispatchResult;

    /// Receive bridge fees from cross-chain operations
    fn receive_bridge_fees(amount: Balance) -> DispatchResult;

    /// Receive slashing fees from validator penalties
    fn receive_slashing_fees(amount: Balance) -> DispatchResult;

    /// Receive stability fees from EDSC operations
    fn receive_stability_fees(amount: Balance) -> DispatchResult;
}
```

#### Treasury Query Interface (For Consensus Day)

```rust
/// Interface for querying accumulated fees (used by Consensus Day)
pub trait TreasuryQueryInterface<Balance> {
    /// Get accumulated transaction fees available for distribution
    fn get_transaction_fee_balance() -> Balance;

    /// Withdraw fees for Consensus Day distribution
    fn withdraw_for_distribution(amount: Balance) -> DispatchResult;
}

/// Implementation for runtime
pub struct TreasuryQueryImpl<T>(PhantomData<T>);

impl<T: Config> TreasuryQueryInterface<BalanceOf<T>> for TreasuryQueryImpl<T> {
    fn get_transaction_fee_balance() -> BalanceOf<T> {
        FundingSourceTotals::<T>::get(FundingSource::TransactionFees)
    }

    fn withdraw_for_distribution(amount: BalanceOf<T>) -> DispatchResult {
        // Verify sufficient balance
        let current = FundingSourceTotals::<T>::get(FundingSource::TransactionFees);
        ensure!(current >= amount, Error::<T>::InsufficientFees);

        // Deduct from fee balance
        FundingSourceTotals::<T>::insert(
            FundingSource::TransactionFees,
            current.saturating_sub(amount)
        );

        // Transfer to Consensus Day distribution
        // (funds already in Treasury account, just accounting update)
...
```rust
/// Treasury management interface
trait TreasuryManagement {
    /// Allocate funds from treasury (governance approved)
    fn spend(
        recipient: AccountId,
        amount: Balance,
        reason: Vec<u8>,
    ...
    /// Get current treasury balance (all sources)
    fn total_balance() -> Balance;

    /// Get balance by funding source
    fn balance_by_source(source: FundingSource) -> Balance;

    /// Set inflation rate (governance controlled)
    fn set_inflation(rate: Perbill) -> DispatchResult;
}

/// Funding sources tracked by Treasury
pub enum FundingSource {
    TransactionFees,    // VMw gas fees (50% of collected)
    BridgeFees,         // Cross-chain bridge fees (10%)
    StabilityFees,      // EDSC collateral/stability fees
    SlashingFees,       // Validator slashing penalties (50%)
    Deposits,           // Registration deposits
    ConsensusDayMint,   // Newly minted funds on Consensus Day
}
```
...
- **Consensus Day distributions** (40% of accumulated fees to participants)
...
- `10-foundation/pallets/pallet-treasury`: Treasury fund management (fee collection, Consensus Day distribution)
- `12-consensus-day`: Annual governance event with hybrid fee distribution
- `08-etwasm-vm`: VMw gas fee collection (50% to Treasury)
- `06-native-currency/etr-token`: ÉTR token with fee routing
- `Runtime Upgrades`: On-chain upgrade coordination
