...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        ///
        /// Used by pallet-consensus-day for hybrid fee distribution
        pub fn get_transaction_fee_balance() -> BalanceOf<T> {
            FundingSourceTotals::<T>::get(FundingSource::TransactionFees)
        }

        /// Get total treasury balance
        pub fn get_total_balance() -> BalanceOf<T> {
            TreasuryBalance::<T>::get()
        }

        /// Withdraw funds from treasury for Consensus Day distribution
        ///
        /// Called by pallet-consensus-day during Distribution phase to
        /// withdraw accumulated fees for participant distribution.
        ///
        /// Note: This does NOT require director approval as it's part of
        /// the automatic Consensus Day distribution mechanism.
        pub fn withdraw_for_consensus_day_distribution(amount: BalanceOf<T>) -> DispatchResult {
            let treasury_account = Self::account_id();
            ...
            // Withdraw from treasury account (funds go to Consensus Day pallet for distribution)
            T::Currency::withdraw(
                &treasury_account,
                amount,
            ...
            // Reset transaction fee accumulator (fees have been distributed)
            FundingSourceTotals::<T>::mutate(FundingSource::TransactionFees, |total| {
...
// ═══════════════════════════════════════════════════════════════════════════════
// Treasury Query Interface Implementation for Consensus Day
// ═══════════════════════════════════════════════════════════════════════════════

/// Wrapper struct for implementing TreasuryQueryInterface
pub struct TreasuryQueryImpl<T>(sp_std::marker::PhantomData<T>);

impl<T: pallet::Config> TreasuryQueryImpl<T> {
    /// Get accumulated transaction fees available for distribution
    pub fn get_transaction_fee_balance() -> <T::Currency as Currency<T::AccountId>>::Balance {
        pallet::Pallet::<T>::get_transaction_fee_balance()
    }

    /// Withdraw funds for Consensus Day distribution
    pub fn withdraw_for_distribution(
        amount: <T::Currency as Currency<T::AccountId>>::Balance
    ) -> frame_support::dispatch::DispatchResult {
        pallet::Pallet::<T>::withdraw_for_consensus_day_distribution(amount)
    }
}
