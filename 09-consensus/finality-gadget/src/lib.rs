...
pub use equivocation::{
    ...
    EquivocationProof,
    EquivocationVote,
    EquivocationVerifyError,
    EquivocationStats,
};
pub use implicit_finality::{
    ImplicitFinalityTracker,
    ImplicitFinalityConfig,
    FinalityStatus,
    ImplicitFinalityState,
};
pub use fork_pruning::{
    ForkPruner,
    ForkPruningConfig,
    PruneStats,
    PruneError,
...
pub struct VoteCollector {
    ...
    view_locked_block: HashMap<View, BlockHash>,
}
...
impl VoteCollector {
    ...
    pub fn add_vote(&mut self, vote: Vote) -> Result<bool, String> {
        ...
        // First block seen for a view wins - reject votes for different blocks
        if let Some(locked_hash) = self.view_locked_block.get(&vote.view) {
            if locked_hash != &vote.block_hash {
                let locked_short = format!("{:02x}{:02x}..{:02x}{:02x}",
                    locked_hash.0[0], locked_hash.0[1], locked_hash.0[30], locked_hash.0[31]);
                let vote_short = format!("{:02x}{:02x}..{:02x}{:02x}",
                    vote.block_hash.0[0], vote.block_hash.0[1], vote.block_hash.0[30], vote.block_hash.0[31]);
                tracing::warn!(
                    "⚠️ V122: Rejecting vote for different block at view {:?}: locked={}, incoming={}",
                    vote.view, locked_short, vote_short
                );
                return Err(format!("View {:?} locked to different block", vote.view));
            }
        } else {
            // Lock this view to the first block seen
            self.view_locked_block.insert(vote.view, vote.block_hash);
            let block_short = format!("{:02x}{:02x}..{:02x}{:02x}",
                vote.block_hash.0[0], vote.block_hash.0[1], vote.block_hash.0[30], vote.block_hash.0[31]);
            tracing::debug!("🔒 V122: Locked view {:?} to block {}", vote.view, block_short);
        }
        ...
        // V9 FIX: Clone validator_id before moving it for display purposes
        let validator_id_clone = vote.validator_id.clone();
        let validator_id_bytes: &[u8; 32] = validator_id_clone.0.as_ref();
...
impl ViewChangeTimer {
    ...
    /// V121: Set view directly for synchronization with network
    /// Used when receiving votes from validators at higher views
...
pub struct FinalityGadget {
    ...
    // V122 FIX: Track certificates already created to prevent spam
    // When quorum is reached (15 votes), we create ONE certificate.
    // Without this, we create a new certificate for every subsequent vote (16, 17, 18...).
    certificates_created: HashSet<(View, BlockHash)>,
}
...
impl FinalityGadget {
    pub fn new(
        ...
        Self {
            ...
            certificates_created: HashSet::new(), // V122: Initialize certificate tracking
        }
    }
    ...
    pub async fn handle_vote(&mut self, vote: Vote) -> Result<(), String> {
        ...
        // 1. Accept votes from any view (no rejection based on view number)
        // 2. If we see a vote from a HIGHER view, adopt that view (sync forward)
        // 3. Votes are still validated by VoteCollector for duplicates/equivocation
        let current_view = self.view_timer.get_current_view();

        // Sync to higher view if we're behind
        if vote.view.0 > current_view.0 {
            log::info!(
                "🔄 View sync: adopting higher view {:?} from vote (was {:?})",
                vote.view, current_view
            );
            self.view_timer.set_view(vote.view);
        }
        ...
        if reached_quorum {
            let cert_key = (vote.view, vote.block_hash);

            // V122 FIX: Check if we already created a certificate for this (view, block)
            // This prevents certificate spam - we only create ONE certificate when quorum is first reached
            if self.certificates_created.contains(&cert_key) {
                // Certificate already created for this view+block, skip
                tracing::debug!(
                    "⏭️ Certificate already created for view={:?}, block={:02x}{:02x}..{:02x}{:02x}, skipping",
                    vote.view,
                    vote.block_hash.0[0], vote.block_hash.0[1],
                    vote.block_hash.0[30], vote.block_hash.0[31]
                );
            ...
            if let Some(signatures) = self.vote_collector.get_quorum_for_block(vote.view, vote.block_hash) {
                ...
                // V122: Mark this (view, block) as having a certificate
                self.certificates_created.insert(cert_key);
    ...
    pub async fn handle_timeout(&mut self) -> Result<(), String> {
        if self.view_timer.should_trigger_view_change() {
            ...
            new_view.sender = self.validator_id.clone();
