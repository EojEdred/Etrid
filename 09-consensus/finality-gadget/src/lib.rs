...
impl CheckpointCertificate {
    /// Calculate the size of this certificate
    pub fn size(&self) -> usize {
        // checkpoint_number: 8 bytes
        // block_hash: 32 bytes
        // authority_set_id: 8 bytes
        // timestamp: 8 bytes
        // signatures: validator_id (4 bytes) + signature (64 bytes Sr25519) per signer
        56 + (self.signatures.len() * 68)
    }

    /// Convert to aggregated BLS certificate (when feature is enabled)
    #[cfg(feature = "bls-aggregation")]
    pub fn to_aggregated(&self, config: &CheckpointConfig) -> Result<AggregatedCheckpointCertificate, String> {
        if !config.use_bls_aggregation {
            return Err("BLS aggregation not enabled".to_string());
        }

        if self.signatures.is_empty() {
            return Err("Cannot aggregate empty certificate".to_string());
        }

        // Create signer bitmap
        let bitmap_size = config.bitmap_size();
        let mut signer_bitmap = vec![0u8; bitmap_size];

        for (validator_id, _) in &self.signatures {
            let byte_index = (validator_id.0 / 8) as usize;
            let bit_index = validator_id.0 % 8;

            if byte_index >= bitmap_size {
                return Err(format!(
                    "Validator ID {} exceeds max validators {}",
                    validator_id.0,
        ...
        // 1. Convert Sr25519 signatures to BLS public keys
        // 2. Aggregate BLS signatures using w3f-bls
        // 3. Verify the aggregated signature
        //
        // For now, create a placeholder aggregated signature
        let aggregated_signature = self.create_mock_bls_signature();

        tracing::info!(
            "📦 Aggregated checkpoint certificate: {} signatures -> {} bytes (from {} bytes, {:.1}% reduction)",
            self.signatures.len(),
            48 + signer_bitmap.len(),
            self.signatures.len() * 64,
            100.0 * (1.0 - (48.0 + signer_bitmap.len() as f64) / (self.signatures.len() as f64 * 64.0))
        );

        Ok(AggregatedCheckpointCertificate {
    ...
    /// Create a mock BLS signature for testing
    #[cfg(feature = "bls-aggregation")]
    fn create_mock_bls_signature(&self) -> Vec<u8> {
        // BLS signatures are 48 bytes
        // In production, this would be generated by w3f-bls library
        use sha2::{Sha256, Digest};

        let mut hasher = Sha256::new();
        hasher.update(&self.checkpoint_number.0.to_le_bytes());
        hasher.update(&self.block_hash.0);
        hasher.update(&self.authority_set_id.0.to_le_bytes());

        let hash = hasher.finalize();

        // BLS signature: 48 bytes (G1 point compressed)
        let mut signature = vec![0u8; 48];
        signature[0..32].copy_from_slice(&hash[..]);
        // Fill remaining 16 bytes with deterministic data
        let timestamp_bytes = self.timestamp.to_le_bytes();
        signature[32..40].copy_from_slice(&timestamp_bytes);
        let sig_count_bytes = self.signatures.len().to_le_bytes();
        signature[40..48].copy_from_slice(&sig_count_bytes);
...
// ============================================================================
// BLS SIGNATURE AGGREGATION TYPES
// ============================================================================

/// Aggregated checkpoint certificate using BLS signatures
/// Size: ~51 bytes vs ~960 bytes for Sr25519 (95% reduction)
#[cfg(feature = "bls-aggregation")]
#[derive(Clone, Debug, Serialize, Deserialize, Encode, Decode)]
pub struct AggregatedCheckpointCertificate {
    ...
    pub block_hash: BlockHash,
    pub authority_set_id: AuthoritySetId,
    /// Single aggregated BLS signature (48 bytes)
    pub aggregated_signature: Vec<u8>,
    /// Bitmap of signers (3 bytes for 21 validators, 1 bit per validator)
...
impl AggregatedCheckpointCertificate {
    /// Calculate the size of the aggregated certificate
    pub fn size(&self) -> usize {
        // checkpoint_number: 8 bytes
        // block_hash: 32 bytes
        // authority_set_id: 8 bytes
        // aggregated_signature: 48 bytes (BLS signature)
        // signer_bitmap: 3 bytes (21 validators / 8 bits)
        // timestamp: 8 bytes
        // Total: ~107 bytes (including metadata)
        // Signature payload: 48 + 3 = 51 bytes
        8 + 32 + 8 + self.aggregated_signature.len() + self.signer_bitmap.len() + 8
    }

    /// Get the number of signers from bitmap
    ...
    pub fn has_signer(&self, validator_id: ValidatorId) -> bool {
        ...
        (self.signer_bitmap[byte_index] & (1 << bit_index)) != 0
    }

    /// Get list of validator IDs who signed
    pub fn get_signers(&self) -> Vec<ValidatorId> {
        let mut signers = Vec::new();
        for (byte_index, byte) in self.signer_bitmap.iter().enumerate() {
            for bit_index in 0..8 {
                if (byte & (1 << bit_index)) != 0 {
                    let validator_id = (byte_index * 8 + bit_index) as u32;
                    signers.push(ValidatorId(validator_id));
                }
            }
        }
        signers
...
/// Configuration for checkpoint finality with BLS aggregation
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct CheckpointConfig {
    /// Enable BLS signature aggregation (feature-gated)
    pub use_bls_aggregation: bool,
    /// Aggregate signatures when this threshold is reached
    pub aggregation_threshold: u32,
    /// Maximum validators for bitmap sizing
...
impl Default for CheckpointConfig {
    fn default() -> Self {
        Self {
            use_bls_aggregation: cfg!(feature = "bls-aggregation"),
            aggregation_threshold: 15, // Aggregate when 15+ signatures (quorum for 21 validators)
            max_validators: 21,
        }
    }
}
...
impl CheckpointConfig {
    /// Calculate bitmap size needed for validators
    pub fn bitmap_size(&self) -> usize {
        ((self.max_validators + 7) / 8) as usize
    }
}
...
impl FinalityGadget {
    ...
    /// Set the keystore for cryptographic signing
    pub fn set_keystore(&mut self, keystore: KeystorePtr) {
    ...
    pub async fn propose_block(&mut self, block_hash: BlockHash) -> Result<Vote, String> {
        // Get current view and timestamp
        ...
        // Create vote message to sign (validator_id, view, block_hash, timestamp)
        let mut message = Vec::new();
        message.extend_from_slice(&self.validator_id.0.to_le_bytes());
        ...
        // Sign using keystore if available
        const ASF_KEY_TYPE: KeyTypeId = KeyTypeId([0x61, 0x73, 0x66, 0x6b]); // "asfk"

        let signature = if let Some(ref keystore) = self.keystore {
            match keystore.sr25519_public_keys(ASF_KEY_TYPE).first() {
                Some(public_key) => {
                    match keystore.sr25519_sign(ASF_KEY_TYPE, public_key, &message_hash) {
                        Ok(Some(sig)) => {
                            tracing::debug!(
                                "✅ Created real Sr25519 signature for vote on block {:?} using validator key",
                                block_hash
                            );
                            sig.0.to_vec()
                        }
                        Ok(None) => {
                            tracing::error!(
                                "❌ Keystore returned None when signing vote - key may not exist"
                            );
                            // Fallback to deterministic signature for testing
                            self.create_deterministic_signature(block_hash)
                        }
                        Err(e) => {
                            tracing::error!(
                                "❌ Failed to sign vote with keystore: {:?}",
                                e
                            );
                            // Fallback to deterministic signature for testing
                            self.create_deterministic_signature(block_hash)
                ...
                None => {
                    tracing::warn!(
                        "⚠️ No ASF validator key found in keystore for vote signing. Using deterministic signature."
                    );
                    self.create_deterministic_signature(block_hash)
                }
            }
            ...
            tracing::warn!(
                "⚠️ No keystore configured for finality gadget. Using deterministic signature for testing."
            );
            self.create_deterministic_signature(block_hash)
        };
    ...
    /// Create a deterministic signature for testing when keystore is unavailable
    fn create_deterministic_signature(&self, block_hash: BlockHash) -> Vec<u8> {
...
pub struct CheckpointFinality {
    ...
    // BLS aggregation (when feature is enabled)
    #[cfg(feature = "bls-aggregation")]
...
impl CheckpointFinality {
    pub fn new(
        ...
        let checkpoint_config = CheckpointConfig {
            use_bls_aggregation: cfg!(feature = "bls-aggregation"),
            aggregation_threshold: quorum_threshold,
            max_validators,
        };

        #[cfg(feature = "bls-aggregation")]
        tracing::info!(
            "🏁 Checkpoint Finality initialized with BLS aggregation: validator={}, quorum={}/{}, bitmap_size={} bytes",
            validator_id.0,
            quorum_threshold,
            max_validators,
            checkpoint_config.bitmap_size()
        );
        ...
        Self {
            ...
            #[cfg(feature = "bls-aggregation")]
            aggregated_certificates: HashMap::new(),
            checkpoint_config,
    ...
    async fn create_checkpoint_certificate(
        ...
        let cert_size = certificate.size();
        tracing::info!(
            "📜 CHECKPOINT CERTIFICATE CREATED: checkpoint={}, block={:?}, signatures={}, size={} bytes",
            checkpoint_number.0,
            winning_hash,
        ...
        // Create aggregated certificate if BLS is enabled
        #[cfg(feature = "bls-aggregation")]
        if self.checkpoint_config.use_bls_aggregation
            && certificate.signatures.len() as u32 >= self.checkpoint_config.aggregation_threshold
        {
            match self.aggregate_signatures(&certificate).await {
                Ok(aggregated) => {
                    let agg_size = aggregated.size();
                    let reduction = 100.0 * (1.0 - (agg_size as f64) / (cert_size as f64));

                    tracing::info!(
                        "📦 BLS AGGREGATED CERTIFICATE: checkpoint={}, size={} bytes (was {} bytes, {:.1}% reduction)",
                        checkpoint_number.0,
                        agg_size,
                        cert_size,
                        reduction
                ...
                Err(e) => {
                    tracing::warn!(
                        "⚠️ Failed to create aggregated certificate for checkpoint {}: {}",
                        checkpoint_number.0,
                        e
                    );
                }
    ...
    // ========== BLS AGGREGATION METHODS ==========

    /// Aggregate signatures into a BLS certificate
    #[cfg(feature = "bls-aggregation")]
    pub async fn aggregate_signatures(
        &self,
        certificate: &CheckpointCertificate,
    ) -> Result<AggregatedCheckpointCertificate, String> {
        if !self.checkpoint_config.use_bls_aggregation {
            return Err("BLS aggregation not enabled".to_string());
        }

        if certificate.signatures.len() < self.checkpoint_config.aggregation_threshold as usize {
            return Err(format!(
                "Insufficient signatures for aggregation: {} (need {})",
                certificate.signatures.len(),
                self.checkpoint_config.aggregation_threshold
            ));
        }

        // Convert to aggregated certificate
        certificate.to_aggregated(&self.checkpoint_config)
    }

    /// Verify an aggregated BLS certificate
    #[cfg(feature = "bls-aggregation")]
    pub async fn verify_aggregated_certificate(
        &self,
        cert: &AggregatedCheckpointCertificate,
    ) -> Result<(), String> {
        ...
        if signer_count < self.quorum_threshold {
            return Err(format!(
                "Certificate has insufficient signers: {} (need {})",
                signer_count,
                self.quorum_threshold
            ));
        }
        ...
        // In production, this would use w3f-bls library to verify:
        // 1. Reconstruct message from checkpoint_number + block_hash + authority_set_id
        // 2. Verify aggregated_signature against message using public keys from bitmap
        // 3. Return Ok(()) if valid, Err() if invalid
        //
        // For now, perform basic validation
        if cert.aggregated_signature.len() != 48 {
            return Err(format!(
                "Invalid BLS signature size: {} (expected 48)",
                cert.aggregated_signature.len()
            ));
        }

        let expected_bitmap_size = self.checkpoint_config.bitmap_size();
        if cert.signer_bitmap.len() != expected_bitmap_size {
            return Err(format!(
                "Invalid signer bitmap size: {} (expected {})",
                cert.signer_bitmap.len(),
        ...
        tracing::debug!(
            "✅ Aggregated certificate verified: checkpoint={}, signers={}",
    ...
    /// Get aggregated certificate if available
    #[cfg(feature = "bls-aggregation")]
    pub fn get_aggregated_certificate(
        &self,
        checkpoint: CheckpointNumber,
    ) -> Option<AggregatedCheckpointCertificate> {
        self.aggregated_certificates.get(&checkpoint).cloned()
    }

    /// Add a received aggregated certificate
    #[cfg(feature = "bls-aggregation")]
    pub async fn add_aggregated_certificate(
        &mut self,
        cert: AggregatedCheckpointCertificate,
    ) -> Result<(), String> {
        // Verify the certificate
        self.verify_aggregated_certificate(&cert).await?;

        let checkpoint_number = cert.checkpoint_number;
        let signer_count = cert.signer_count();

        // Store the certificate
        self.aggregated_certificates.insert(checkpoint_number, cert.clone());

        tracing::info!(
            "📦 Received aggregated certificate: checkpoint={}, signers={}, size={} bytes",
            checkpoint_number.0,
            signer_count,
            cert.size()
        );

        // If we don't have a regular certificate, we can reconstruct it from aggregated
        if !self.checkpoint_certificates.contains_key(&checkpoint_number) {
            // Create a minimal certificate entry for tracking
            let minimal_cert = CheckpointCertificate {
                checkpoint_number: cert.checkpoint_number,
                block_hash: cert.block_hash,
                authority_set_id: cert.authority_set_id,
                signatures: Vec::new(), // We don't have individual signatures
                timestamp: cert.timestamp,
            };

            self.checkpoint_certificates.insert(checkpoint_number, minimal_cert);
            self.metrics.total_certificates += 1;
        }
    ...
    pub fn update_checkpoint_config(&mut self, config: CheckpointConfig) {
        tracing::info!(
            "🔧 Checkpoint config updated: bls_enabled={}, threshold={}, max_validators={}",
            config.use_bls_aggregation,
...
mod tests {
    ...
    #[cfg(feature = "bls-aggregation")]
    #[tokio::test]
    async fn test_bls_certificate_aggregation() {
        let client = Arc::new(MockCanonicalChainClient::new());
        let bridge = Arc::new(MockNetworkBridge);
        let mut checkpoint = CheckpointFinality::new(ValidatorId(0), 21, client, bridge);
        ...
        // Add 15 signatures to reach aggregation threshold
        for i in 0..15 {
            let sig = CheckpointSignature {
                validator_id: ValidatorId(i),
                checkpoint_number: CheckpointNumber(1),
        ...
        // Check that aggregated certificate was created
        let agg_cert = checkpoint.get_aggregated_certificate(CheckpointNumber(1));
        assert!(agg_cert.is_some(), "Aggregated certificate should be created");

        let agg_cert = agg_cert.unwrap();
        assert_eq!(agg_cert.signer_count(), 15);
        assert_eq!(agg_cert.aggregated_signature.len(), 48); // BLS signature size
        assert_eq!(agg_cert.signer_bitmap.len(), 3); // 21 validators / 8 bits = 3 bytes
    }

    #[cfg(feature = "bls-aggregation")]
    #[tokio::test]
    async fn test_bls_bitmap_encoding() {
        ...
        let agg_cert = cert.to_aggregated(&config).unwrap();

        // Verify bitmap has correct bits set
        for i in 0..15 {
            assert!(agg_cert.has_signer(ValidatorId(i)), "Validator {} should be in bitmap", i);
        }

        for i in 15..21 {
            assert!(!agg_cert.has_signer(ValidatorId(i)), "Validator {} should NOT be in bitmap", i);
        }

        // Verify get_signers returns correct list
        let signers = agg_cert.get_signers();
        assert_eq!(signers.len(), 15);
        for i in 0..15 {
            assert!(signers.contains(&ValidatorId(i)));
        }
    }

    #[cfg(feature = "bls-aggregation")]
    #[tokio::test]
    async fn test_bls_size_reduction() {
        let config = CheckpointConfig {
            use_bls_aggregation: true,
            aggregation_threshold: 15,
            max_validators: 21,
        ...
        let original_size = cert.size();
        let agg_cert = cert.to_aggregated(&config).unwrap();
        let agg_size = agg_cert.size();

        // Verify significant size reduction
        let reduction = 100.0 * (1.0 - (agg_size as f64) / (original_size as f64));
        assert!(reduction > 90.0, "Should have >90% size reduction, got {:.1}%", reduction);

        // Original: 56 + (15 * 68) = 1076 bytes
        // Aggregated: ~107 bytes (checkpoint metadata + 48 byte sig + 3 byte bitmap)
        assert!(original_size > 1000, "Original size should be >1000 bytes");
        assert!(agg_size < 150, "Aggregated size should be <150 bytes");
    }

    #[cfg(feature = "bls-aggregation")]
    #[tokio::test]
    async fn test_bls_certificate_verification() {
        let client = Arc::new(MockCanonicalChainClient::new());
        let bridge = Arc::new(MockNetworkBridge);
        let checkpoint = CheckpointFinality::new(ValidatorId(0), 21, client.clone(), bridge);

        let config = checkpoint.get_checkpoint_config();
        ...
        let result = checkpoint.verify_aggregated_certificate(&agg_cert).await;
        assert!(result.is_ok(), "Valid aggregated certificate should pass verification");

        // Test insufficient signers
        let mut bad_cert = agg_cert.clone();
        bad_cert.signer_bitmap = vec![0x01, 0x00, 0x00]; // Only 1 signer
        let result = checkpoint.verify_aggregated_certificate(&bad_cert).await;
        assert!(result.is_err(), "Certificate with insufficient signers should fail");

        // Test invalid signature size
        let mut bad_cert = agg_cert.clone();
        bad_cert.aggregated_signature = vec![0; 32]; // Wrong size
        let result = checkpoint.verify_aggregated_certificate(&bad_cert).await;
        assert!(result.is_err(), "Certificate with invalid signature size should fail");

        // Test wrong authority set
        let mut bad_cert = agg_cert.clone();
        bad_cert.authority_set_id = AuthoritySetId(999);
        let result = checkpoint.verify_aggregated_certificate(&bad_cert).await;
        assert!(result.is_err(), "Certificate with wrong authority set should fail");
    }

    #[cfg(feature = "bls-aggregation")]
    #[tokio::test]
    async fn test_bls_add_aggregated_certificate() {
        let client = Arc::new(MockCanonicalChainClient::new());
        let bridge = Arc::new(MockNetworkBridge);
        let mut checkpoint = CheckpointFinality::new(ValidatorId(0), 21, client.clone(), bridge);
        ...
        // Add the aggregated certificate
        let result = checkpoint.add_aggregated_certificate(agg_cert.clone()).await;
        assert!(result.is_ok(), "Should successfully add aggregated certificate");

        // Verify it was stored
        let stored = checkpoint.get_aggregated_certificate(CheckpointNumber(1));
        assert!(stored.is_some(), "Aggregated certificate should be stored");

        // Verify regular certificate was also created
        let regular_cert = checkpoint.get_certificate(CheckpointNumber(1));
        assert!(regular_cert.is_some(), "Regular certificate should be created from aggregated");
    }
    ...
    fn test_checkpoint_config() {
        ...
        assert_eq!(config.aggregation_threshold, 15);
        assert_eq!(config.bitmap_size(), 3); // 21 validators / 8 = 3 bytes

        // Test custom config
        let custom = CheckpointConfig {
            use_bls_aggregation: true,
            aggregation_threshold: 10,
            max_validators: 100,
        };
        assert_eq!(custom.bitmap_size(), 13); // 100 validators / 8 = 13 bytes
    }

    #[cfg(feature = "bls-aggregation")]
    #[test]
    fn test_bitmap_signer_operations() {
        let config = CheckpointConfig {
            use_bls_aggregation: true,
        ...
        let mut signatures = Vec::new();
        // Add validators 0, 2, 4, 6, 8 (sparse pattern)
        for i in [0, 2, 4, 6, 8] {
            signatures.push((ValidatorId(i), vec![i as u8; 64]));
        }
        ...
        // Verify sparse signers
        assert!(agg_cert.has_signer(ValidatorId(0)));
        assert!(!agg_cert.has_signer(ValidatorId(1)));
        assert!(agg_cert.has_signer(ValidatorId(2)));
        assert!(!agg_cert.has_signer(ValidatorId(3)));
        assert!(agg_cert.has_signer(ValidatorId(4)));
        assert!(!agg_cert.has_signer(ValidatorId(5)));
        assert!(agg_cert.has_signer(ValidatorId(6)));
        assert!(!agg_cert.has_signer(ValidatorId(7)));
        assert!(agg_cert.has_signer(ValidatorId(8)));
        assert!(!agg_cert.has_signer(ValidatorId(9)));

        assert_eq!(agg_cert.signer_count(), 5);

        let signers = agg_cert.get_signers();
        assert_eq!(signers.len(), 5);
        assert!(signers.contains(&ValidatorId(0)));
        assert!(signers.contains(&ValidatorId(2)));
        assert!(signers.contains(&ValidatorId(4)));
        assert!(signers.contains(&ValidatorId(6)));
        assert!(signers.contains(&ValidatorId(8)));
    }
}